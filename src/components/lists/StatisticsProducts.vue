...
const list = computed(() => store.getters.getStatisticProducts);
...
function fetchData() {
  store.dispatch('fetchStatisticsProducts', form);
}
</script>

<template>
  <div class="flex">
    <CafeSelect class="md:w-80" @update:model-value="changeCafe" />

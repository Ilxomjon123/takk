...
<template>
  <div class="flex flex-col md:flex-row flex-wrap w-full">
    <div
      ...
      <input type="file" ref="inputFile" multiple class="hidden" @change="addImage" />
    </div>
    <div v-for="item, itemIndex in imageSources"
      class="md:basis-1/2 lg:basis-1/4 2xl:basis-1/6 border-2 border-dashed shadow-sm border-gray-200 dark:border-dark-5 rounded-md p-5">
      <div class="h-24 lg:h-40 relative image-fit cursor-pointer zoom-in mx-auto">
        <img class="rounded-md" alt="Logo" :src="item.image" data-action="zoom" />
        <Tippy tag="div" content="Remove this logo?"
...
.toastify_warning {
  background: rgba(0, 0, 0, 0) linear-gradient(to right,
      rgb(255, 0, 0),
      rgb(255, 95, 110),
      rgb(255, 195, 113)) repeat scroll 0% 0% !important;
